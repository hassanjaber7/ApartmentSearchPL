...
// Searching for rooms in Warsaw
  ...
  console.log(listingsRooms.length);
  for (const listingRoom of listingsRooms) {
    const title = await listingRoom.locator('[data-nx-name="H4"]').textContent();
    const price = await listingRoom.locator('[data-testid="ad-price"]').textContent();
    const locationDate = await listingRoom.locator('[data-testid="location-date"]').textContent();
    const link = await listingRoom.locator('[data-testid="card-title-link"]').getAttribute('href');
    const baseUrl = 'https://www.olx.pl/';
    const fullLink = baseUrl + link;
...
// Merge the new listings with existing ones and save to JSON
 const allDataRooms = [...existingListings, ...listingData];
 fs.writeFileSync('listings.json', JSON.stringify(allDataRooms, null, 2));
  
  console.log('💾 Saved to listings.json');
  // Save the new listings to a separate JSON file for Telegram
  const jsonDataRooms = JSON.stringify(listingData, null, 2);
  fs.writeFileSync('MostRecentListings.json', jsonDataRooms);
